...
describe("CurrencyConverter", function () {
  ...
  describe("with mocks", function () {
    ...
    it("does call API with correct parameters", () => {
      ...
      expect(conversionRateApi.getRate).toHaveBeenCalledWith(
        CurrencyIsoCode.USD,
        CurrencyIsoCode.EUR
      );
    });
  ...
  describe("with fakes", function () {
    it("should perform a conversion", function () {
      const conversionRateApiFake = new ConversionRateApiFake().withRate(
        CurrencyIsoCode.USD,
        CurrencyIsoCode.EUR,
        2
      );
    ...
    it("should not call twice the API if conversion is the same", function () {
      ...
      expect(conversionRateApiFake.getRate).toHaveBeenCalledWith(
        CurrencyIsoCode.USD,
        CurrencyIsoCode.EUR
      );
    });
    ...
    it("should not call twice the API if conversion is the same (full fake)", function () {
      ...
      expect(conversionRateApiFake.getRatesCalls[0]).toEqual({
        from: CurrencyIsoCode.USD,
        to: CurrencyIsoCode.EUR,
      });
    });
  });
});
